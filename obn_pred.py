...
from obn_embed import embed_paired
...
def predict_all_pairs(embed_root: str, output_csv: str):
    ...
    prot_ids = sorted(os.path.splitext(f)[0] for f in os.listdir(prot_dir) if f.endswith(".npz"))
    lig_ids  = sorted(os.path.splitext(f)[0] for f in os.listdir(lig_dir)  if f.endswith(".npz"))

    if not prot_ids or not lig_ids:
        print("❌ 단백질 또는 리간드 임베딩이 비어 있습니다."); return

    print(f"총 조합 수: {len(prot_ids)} x {len(lig_ids)} = {len(prot_ids) * len(lig_ids)}")
    ...
    rows = []
    for pid in tqdm(prot_ids, desc="단백질"):
        p_vec = load_npz_vec(os.path.join(prot_dir, f"{pid}.npz"))
        for lid in lig_ids:
            l_vec = load_npz_vec(os.path.join(lig_dir, f"{lid}.npz"))
...
if __name__ == "__main__":
    main(
        ...
        output_path  = "./sample/obn/run_output",
