...
def embed_paired(
    ...
    out_fa, out_sm = (os.path.join(out_root, "obn_prot"),
                      os.path.join(out_root, "obn_lig"))
    ...
    # ===== 단백질 임베딩 =====
    print(f"[단백질] 총 {len(fasta)}개 시퀀스 임베딩")
    tok_p = AutoTokenizer.from_pretrained(prot_model)
    mdl_p = AutoModel.from_pretrained(prot_model).to(dev).eval()

    prot_ids = list(fasta.keys())
    for i in range(0, len(prot_ids), bs_prot):
        ids = prot_ids[i:i + bs_prot]
    ...
    # ===== SMILES 임베딩 =====
    print(f"[화합물] 총 {len(smiles)}개 SMILES 임베딩")
    tok_s = AutoTokenizer.from_pretrained(chem_model)
    mdl_s = AutoModel.from_pretrained(chem_model).to(dev).eval()

    smi_ids = list(smiles.keys())
    for i in range(0, len(smi_ids), bs_chem):
        ids = smi_ids[i:i + bs_chem]
    ...
    print(f"완료! 단백질 {len(fasta)}개, 화합물 {len(smiles)}개 임베딩 저장")

# ----------- 사용 예시 ----------- #
# embed_paired(
#     fasta_path="./sample/sample.fasta",
#     smi_path="./sample/sample.smi",
#     out_root="./sample/obn/sample_embeddings"
# )
